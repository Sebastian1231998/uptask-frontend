...
const Busqueda = () => {
    ...
    const {modalBuscador, setModalBuscador, proyectos} = useProyecto()

    const proyectosFiltrados = proyectos.filter(proyecto => proyecto.nombre.toLowerCase().includes(busqueda.toLowerCase()) )

    return (
        <Transition.Root show={ modalBuscador  } as={Fragment} afterLeave={ ()=> setBusqueda('') }>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={ () => setModalBuscador(false)}>
                <Transition.Child
                    as={Fragment}
                ...
                <Combobox
                    ...
                    onChange={(proyecto)=> window.location = `/main/proyectos/${proyecto._id}`}
                >
